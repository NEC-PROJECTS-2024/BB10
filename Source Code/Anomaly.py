...
Networkanomaly_model = pickle.load(open('Anomaly.sav', 'rb'))

# sidebar for navigation
with st.sidebar:
    selected = option_menu('Network Anomaly Detection System Using Machine Learning',
                          ['Home'],
                          default_index=0)

# Network Anomaly Detection  Page
if selected == 'Home':
    # page title
    st.title('Network Anomaly Detection System Using Machine Learning')
    ...
    # code for Prediction
NetworkAnomaly_Detection = ''

    # creating a button for Prediction
if st.button('Network Anomaly Detection Result'):
        ...
        NetworkAnomaly_prediction = Networkanomaly_model.predict([user_input])
        st.success('The output is {}'.format( NetworkAnomaly_prediction))
